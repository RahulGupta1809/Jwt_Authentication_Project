...
public class UserImpl implements UserRepository {
	...
	public ResponseDto getUser(int userId) {
		...
		ResponseEntity<UserDto> responseEntity = restTemplate
				.getForEntity("http://192.168.60.16:8081/api/users/" + userId, UserDto.class);
	...
	/**
	 * @Override public ArrayList<ResponseDto> getUsers() { ArrayList<ResponseDto>
	 *           responseDto = new ResponseDto<>(); ResponseEntity<UserDto>
	 *           responseEntity = restTemplate
	 *           .getForEntity("http://192.168.60.16:8081/api/user" ,
	 *           UserDto.class);
	 * 
	 *           ArrayList<UserDto> userDto = responseEntity.getBody();
	 *           responseDto.setUser(userDto); return responseDto; }
	 */
